...
colnames(stimuli) <- c("Type","Pair","ID","Det.","N0","Verb","Det.2","N1","Prep.","Det.3","Adj.","N2","Attachment","Unambiguous","fullID")
#load response file
...
percent_VAresponses <- attachment_percent[attachment_percent$response_attachment=="Verb",]
percent_NAresponses <- attachment_percent[attachment_percent$response_attachment=="Nomen",]
#ggplot(attachment_percent, aes(percent, fill = response_attachment)) + 
#  geom_density(alpha = 0.2)
ggplot(attachment_percent, aes(percent, fill = response_attachment)) + geom_histogram(alpha = 0.5, aes(y = ..density..), position = 'identity')
```

```{r echo=FALSE}
t.test( percent_NAresponses$percent,percent_VAresponses$percent,paired=TRUE)
```
...
## Reject stimuli with too few correct responses
```{r}
numericInput("threshold", "Choose accuracy threshold to reject items", 0.75)
...
DT::renderDataTable({
  items_reject <- df.stim_acc %>% arrange(Verb) %>% filter(acc <= input$threshold)
  #datatable(items_reject[,c(1,2,5:12,16)],
  #          filter = 'top',
