...
datatable(df.stim_acc[df.stim_acc$fullID %in% df.responses_clean$item,c(1,2,4,6:13,17,18)])
```
There are `r nrow(stimuli[stimuli$fullID %in% df.responses_clean$items & stimuli$Type==1,])` items of Type 1 left  
There are `r nrow(stimuli[stimuli$fullID %in% df.responses_clean$items & stimuli$Type==2,])` items of Type 2 left

## 3. Biases
### 3a. Accuracy difference within pairs
```{r,warning=FALSE,message=FALSE}
...
p <- ggplot(summary_acc, aes(x = factor(attachment), y = mean_acc,subject = subjects,fill=factor(attachment))) +
    geom_boxplot() +
    geom_jitter(size = 2) +
    ggtitle("Accuracy averaged over items")
p <- ggplotly(p,tooltip = c("subject","mean_rt"))
p
```
